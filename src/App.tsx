...
export function App() {
  ...
  async function confirmImport(reviewed: ParsedScheduleDraft) {
    ...
    try {
      ...
      setSelectedListIds(
        Object.fromEntries(defaultLists.map((list) => [list.departmentId, list.id]))
      );
      setAssignments([]);
      setDraft(null);
      setView("positions");
  ...
  function createPositionList(name: string) {
    ...
    const baseId = `${currentDepartmentId}-${label.toLowerCase().replace(/[^a-z0-9]+/g, "-").replace(/^-|-$/g, "") || "list"}`;
    ...
    <Shell>
      ...
      {view === "positions" ? (
        <PositionListEditor
          ...
          onSelectList={(listId) =>
            setSelectedListIds((current) => ({ ...current, [currentDepartmentId]: listId }))
          }
